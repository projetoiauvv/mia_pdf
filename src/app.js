...
function parseConversationContent(content) {
  ...
  try {
    ...
    const mergedTopLevelArrays = normalizedContent.replace(/\]\s*\[/g, ',');
    if (mergedTopLevelArrays !== normalizedContent) {
...
function renderPreview() {
  elements.messageCount.textContent = `${state.messages.length} ${state.messages.length === 1 ? 'mensagem' : 'mensagens'}`;
...
function loadConversationPayload(payload) {
...
async function handleFileUpload(event) {
  ...
  try {
    loadConversationPayload(parseConversationContent(await file.text()));
  } catch (error) {
    state.attendantName = '';
    state.messages = [];
    renderPreview();
